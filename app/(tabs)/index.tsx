...
import { Image, StyleSheet, Platform, Text, View, SafeAreaView, TextInput, Button, ActivityIndicator, FlatList } from 'react-native';
...
export default function HomeScreen() {
  ...
  const renderCharacter = ({item} : {item : Character}) => (
    <Card title={item.name} description={item.status} image={item.image} />
  )
      ...
      <View style={{flex: 1}}>
        <View style={{alignItems: 'center'}}>
          <View style={styles.inputContainer}>
            <TextInput style={styles.input}  onChangeText={(text) => setPage(text)} keyboardType='numeric' placeholder='1/42' />
